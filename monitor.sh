...
: "${WARN_ENV_MISSING:="WARN: .env не найден: {FILE}. Используются defaults."}"
: "${WARN_ENV_INSECURE:="WARN: .env не загружен (небезопасные права): {FILE} ({REASON})"}"
: "${LOG_ENV_MISSING:=".env не найден: {FILE}. Используются defaults."}"
: "${LOG_ENV_INSECURE:=".env не загружен (небезопасные права): {FILE} ({REASON})"}"
: "${ERR_CMD_REQUIRED:="FATAL: {CMD} обязателен"}"
: "${ERR_UNSAFE_PATH:="FATAL: небезопасный path {FILE} (dir={DIR} reason={REASON})"}"
: "${ERR_FATAL_STDERR:="[FATAL] line={LINE} cmd='{CMD}' exit={EXIT}"}"
: "${LOG_FATAL_ON_ERROR:="{HOST}:{SCRIPT}:{LINE} cmd='{CMD}' exit={EXIT}"}"

: "${WARN_TG_NOT_CONFIGURED:="WARN: Telegram не настроен (TG_TOKEN/TG_CHAT пустые)"}"
: "${WARN_URL_MISSING_HTTP_ENABLED:="WARN: ENABLE_HTTP_MONITOR=1, но URLS не задан"}"
: "${WARN_URL_AND_NO_WEB:="WARN: URLS не задан и web-сервис не обнаружен"}"

: "${LOG_TG_NOT_CONFIGURED:="Telegram не настроен (TG_TOKEN/TG_CHAT пустые)"}"
: "${LOG_HTTP_FAIL:="HTTP-проверка не прошла для {URL}: code={CODE}, fails={FAILS}, threshold={THRESHOLD}"}"
: "${LOG_URL_DOWN_SERVICE_UP:="URL недоступен для {URL}, service {WEB} запущен"}"
: "${LOG_URL_DOWN_SERVICE_DOWN:="URL недоступен для {URL}, service {WEB} не запущен"}"
: "${LOG_URL_DOWN_NO_SERVICE:="URL недоступен для {URL}, web-сервис не обнаружен"}"
: "${LOG_HTTP_UP_SERVICE_DOWN:="URL доступен для {URL}, но services остановлены: {WEB_INACTIVE}"}"
: "${LOG_HTTP_UP_SERVICE_RECOVER:="URL доступен для {URL}, services восстановлены: {WEB_ACTIVE}"}"
: "${LOG_METRIC_ALERT:="{NAME} alert: {VALUE}%"}"
: "${LOG_CONTAINER_STOPPED:="Контейнер {CT} остановлен"}"
: "${LOG_SERVICE_RECOVER_HTTP_DOWN:="Service {WEB} восстановлен, но HTTP все еще недоступен для {URLS}"}"
: "${LOG_WEB_ACTIVE:="Web services активны: {WEB_ACTIVE}"}"
: "${LOG_WEB_INACTIVE:="Web services неактивны: {WEB_INACTIVE}"}"
: "${LOG_ROTATED:="Лог ротирован (оставлено последних строк: {LINES})"}"
: "${LOG_STATE_CREATED:="State file создан"}"
: "${LOG_TG_SEND_FAILED:="Не удалось отправить Telegram-сообщение"}"
: "${LOG_HTTP_DOWN_CONFIRMED:="HTTP DOWN подтвержден для {URL} (code={CODE})"}"
: "${LOG_HTTP_RECOVERED:="HTTP восстановился для {URL}, time={TIME}s, code={CODE}"}"
: "${LOG_METRIC_RECOVERED:="{NAME} восстановился: {VALUE}%"}"
: "${LOG_CONTAINER_STARTED:="Контейнер {CT} снова запущен"}"
: "${LOG_MONITOR_STARTED:="Запуск monitor run"}"
: "${LOG_MONITOR_FINISHED:="Monitor run завершен"}"
: "${LOG_HTTP_AND_SERVICE_DOWN:="HTTP уже недоступен для {URLS}, и service {WEB} теперь остановлен"}"
: "${LOG_ANOTHER_INSTANCE:="Другой экземпляр уже запущен, завершаю работу"}"
...
: "${ENABLE_WEB_SERVICE_DETECTION:=1}"
...
if [[ "${ENABLE_WEB_SERVICE_DETECTION:-1}" -eq 1 ]]; then
  detect_web_service
  active_json="[]"
  inactive_json="[]"
  ...
  WEB_ACTIVE_UNITS=()
  WEB_INACTIVE_UNITS=()
  if [[ "$prev_web_down" == "true" || -n "$prev_web_name" || -n "$prev_web_active" || -n "$prev_web_inactive" ]]; then
    state_apply ".web.down = \$flag | .web.name = \$name | .web.services.active = [] | .web.services.inactive = []" \
      --argjson flag false --arg name ""
  fi
fi
...
if [[ "${ENABLE_WEB_SERVICE_DETECTION:-1}" -eq 1 && "$WEB_FOUND" == "false" && -z "${URLS//[[:space:]]/}" ]]; then
